...
def put(canvas, image, a, b, alignment="00"):
    canvas.alpha_composite(image, (int(
        a)-(image.size[0] * int(alignment[0]) // 2), int(b)-(image.size[1] * int(alignment[1]) // 2)))
    return canvas
...
def gradient(width, height, colora, colorb):
    r = Image.frombytes("L", (width, 1), np.uint8(
        np.linspace(colora[0], colorb[0], width)))
    g = Image.frombytes("L", (width, 1), np.uint8(
        np.linspace(colora[1], colorb[1], width)))
    b = Image.frombytes("L", (width, 1), np.uint8(
        np.linspace(colora[2], colorb[2], width)))
    ...
    if (underline):
        ...
        if (i == "\n"):
            ...
            whitechar = put(whitechar, Image.new(
                "RGBA", (w(char), 1), (255, 255, 255, 255)), 0, h(char)-2+underlineoffset)
            ...
            alpha2 = ImageMath.eval("convert( int( (r1-r2+255+g1-g2+255+b1-b2+255)/3*alp/255 ), 'L')",
                                    r1=cred, r2=wcred, b1=cblue, b2=wcblue, g1=cgreen, g2=wcgreen, alp=(color[3]))
...
def createtext7(im, x, y, text, fontdirectory, color=(0, 0, 0, 255), buffersize=(3000, 3000), align="00", kerningadjust=0, fit=9999999):
    global buffercache
    drawntext = getfromcache(buffercache, str(
    ...
    drawntext = put(Image.new("RGBA", (w(im), h(im)),
                    (0, 0, 0, 0)), drawntext, x, y, align)
    whitedrawntext = whitedrawntext.crop((0, 0, width, height))
    whitedrawntext = put(Image.new("RGBA", (w(im), h(im)),
                         (0, 0, 0, 0)), whitedrawntext, x, y, align)
    ...
    r = ImageMath.eval(
        "convert( b*c/255+(255-w)*(255-c)/255 ,'L')", w=r, b=wr, c=c[0])
    g = ImageMath.eval(
        "convert( b*c/255+(255-w)*(255-c)/255 ,'L')", w=g, b=wg, c=c[1])
    b = ImageMath.eval(
        "convert( b*c/255+(255-w)*(255-c)/255 ,'L')", w=wb, b=b, c=c[2])
    # imgcolor.show()
    # drawntext.show()
    ...
    green = ImageMath.eval(
        "convert( int(((i*(255-t)/255+(c*t)/255)*a/255+i*(255-a)/255)*o/255+(i*(255-o))/255) , 'L')", i=ig, t=g, c=c[1], a=a, o=c[3])
    blue = ImageMath.eval(
        "convert( int(((i*(255-t)/255+(c*t)/255)*a/255+i*(255-a)/255)*o/255+(i*(255-o))/255) , 'L')", i=ib, t=b, c=c[2], a=a, o=c[3])
    alpha = ImageMath.eval("convert( int(((((r+g+b)/3+(255-(r+g+b)/3)*i/255))*t/255+(i*(255-t))/255)*o/255+(i*(255-o))/255) , 'L')",
                           i=ia, r=r, g=g, b=b, t=a, o=c[3])  # i is the image alpha,  r,g,b are RGB values of the text,  t is text alpha,  o is color alpha
    result = Image.merge("RGBA", (red, green, blue, alpha))
    return result


# this gives width and height of text using windows 7 rendering
def measuretext7(text, fontdirectory, buffersize=(3000, 3000), kerningadjust=0, fit=9999999):
    # drawntext = Image.new("RGBA",buffersize,(255,127,127,0))
    width = 0
    height = 0
    line = 0
...
# this resizes image but keeps margins intact. think of Unity GUI elements
def resize(im, width, height, left, right, up, down, scalingmethod=Image.NEAREST):
    if width < w(im):
    ...
    result = put(result, tm.resize(
        (width-left-right, h(tm)), scalingmethod), left, 0)
    result = put(result, tr, width, 0, "20")
    result = put(result, ml.resize(
        (w(ml), height-up-down), scalingmethod), 0, up)
    result = put(result, mm.resize(
        (width-left-right, height-up-down), scalingmethod), left, up)
    result = put(result, mr.resize(
        (w(mr), height-up-down), scalingmethod), width, up, "20")
    result = put(result, dl, 0, height, "02")
    result = put(result, dm.resize((width-left-right, h(dm)),
                 scalingmethod), left, height, "02")
    result = put(result, dr, width, height, "22")
    return result


# this is resize, but you give it desired coordinates and it calculates the size the image should be
def resizeanchor(im, x1, y1, x2, y2, left, right, up, down, scalingmethod=Image.NEAREST):
...
def Create3_1Button(text, style=0, underline=False):
    ...
    if style == 1:
        ...
        Button = put(Button, BorderImg.crop((textx-2,      6,
                     textx-1,            7+h(textgraphic))),   textx-2,    6)
        Button = put(Button, BorderImg.crop((textx-1,      7+h(textgraphic),
                     textendx,           7+h(textgraphic)+1)), textx-1,    7+h(textgraphic))
        Button = put(Button, BorderImg.crop((textendx+1,   6,
                     textendx+2,         7+h(textgraphic))),   textendx+1, 6)
        Button = put(Button, BorderImg.crop((textx-1,      5,
                     textendx,           6)),                  textx-1,    5)
    else:
        Button = resize(Button, max(58, w(textgraphic)+6+6),
                        h(textgraphic)+6+6, 3, 3, 3, 3)
        Button = put(Button, textgraphic, floor(
            w(Button)/2-w(textgraphic)/2-1), 6, "00")
    return Button
...
def Create95Button(text, style=0, underline=False):
    ...
    if style == 1:
        ...
        Button = put(Button, BorderImg.crop((outx,      4,
                     outx+1,            6+h(textgraphic))),   outx,    4)
        Button = put(Button, BorderImg.crop((outx,      5+h(textgraphic),
                     outendx,           5+h(textgraphic)+1)), outx,    5+h(textgraphic))
        Button = put(Button, BorderImg.crop((outendx-1,   4,
                     outendx,         6+h(textgraphic))),   outendx-1, 4)
        Button = put(Button, BorderImg.crop((outx,      4,
                     outendx,           5)),                  outx,    4)
    else:
        Button = resize(Button, max(75, w(textgraphic)+5+5),
                        h(textgraphic)+4+6, 2, 2, 2, 2)
        Button = put(Button, textgraphic, floor(
            w(Button)/2-w(textgraphic)/2), 4)
    return Button
...
def CreateXPWindow(param):
    ...
    if len(param.buttons) > 0:
        ...
        button1img = CreateXPButton(
            param.buttons[0], param.buttonstyles[0]*(param.buttonstyles[0] != 4))
        # IMAGE = put(IMAGE,button1img,3+12,height-3-12,"02")
        buttonsheight = max(buttonsheight, h(button1img)+14)
        temp = Image.new("RGBA", (buttonswidth+w(button1img),
                         buttonsheight), (0, 0, 0, 0))
        ...
        for i in range(len(param.buttons)-1):
            buttonswidth += 6
            button2img = CreateXPButton(
                param.buttons[i+1], param.buttonstyles[i+1]*(param.buttonstyles[i+1] != 4))
            # IMAGE = put(IMAGE,button2img,3+12,height-3-12,"02")
            buttonsheight = max(buttonsheight, h(button2img)+14)
            temp = Image.new(
                "RGBA", (buttonswidth+w(button2img), buttonsheight), (0, 0, 0, 0))
            temp = put(temp, buttonsimage, 0, 0)
    ...
    IMAGE = put(IMAGE, resize(TopFrame, width, h(
        TopFrame), 28, 35, 9, 17, Image.NEAREST), 0, 0)
    IMAGE = put(IMAGE, LeftFrame.resize(
        (3, height-h(TopFrame)-3), Image.NEAREST), 0, h(TopFrame), "00")
    IMAGE = put(IMAGE, RightFrame.resize((3, height-h(TopFrame)-3),
                Image.NEAREST), width, h(TopFrame), "20")
    IMAGE = put(IMAGE, cropx(BottomFrame, 0, 5).resize(
        (5, 3), Image.NEAREST), 0, height, "02")
    IMAGE = put(IMAGE, cropx(BottomFrame, 4, w(BottomFrame) -
                5).resize((width-10, 3), Image.NEAREST), 5, height, "02")
    IMAGE = put(IMAGE, cropx(BottomFrame, w(BottomFrame)-5, w(BottomFrame)
                             ).resize((5, 3), Image.NEAREST), width, height, "22")
    IMAGE = put(IMAGE, Image.new("RGBA", (width-6, height-3 -
                h(TopFrame)), (236, 233, 216, 255)), 3, h(TopFrame), "00")
...
def CreateMacAlertDialog(width, height, title="", bar=True, icon="", errortext="", subtext="", button1="", button2="", button3="", button1default=False, button2default=False, button3default=False, button1style=0, button2style=0, button3style=0):
    ...
    if (bar):
        ...
        IMAGE = put(IMAGE, resize(
            WindowNoBar, width, height, 3, 4, 3, 4), 0, 0)
    if bar:
        if (title == ""):
            ...
            IMAGE = put(IMAGE, resizeanchor(Ridges, width//2+w(TitleImage) //
                        2+5, 4, width-6, 16, 1, 1, 1, 1), width//2+w(TitleImage)//2+5, 4)
    if (icon != ""):
    ...
    if (button1 != ""):
        ...
        if (button1default):
            button1border = resize(ButtonBorder, w(
                button1img)+6, h(button1img)+6, 5, 5, 5, 5)
            IMAGE = put(IMAGE, button1border, width-17+3, height-17+3, "22")
        if (button2 != ""):
            button2img = CreateMacButton(button2, button2style)
            IMAGE = put(IMAGE, button2img, width-17 -
                        w(button1img)-22, height-17, "22")
            if (button2default):
                button2border = resize(ButtonBorder, w(
                    button2img)+6, h(button2img)+6, 5, 5, 5, 5)
                IMAGE = put(IMAGE, button2border, width-17+3 -
                            w(button1img)-22, height-17+3, "22")
            if (button3 != ""):
                button3img = CreateMacButton(button3, button3style)
                IMAGE = put(IMAGE, button3img, width-17 -
                            w(button2img)-22-w(button1img)-22, height-17, "22")
                if (button3default):
                    button3border = resize(ButtonBorder, w(
                        button3img)+6, h(button3img)+6, 5, 5, 5, 5)
                    IMAGE = put(IMAGE, button3border, width-17+3 -
                                w(button2img)-22-w(button1img)-22, height-17+3, "22")
    return IMAGE
...
def CreateMacWindoid(icon="", text="", collapsed=False):
    ...
    if not collapsed:
        ...
        IMAGE = put(IMAGE, resize(CollapsedBorder,
                    15, height, 2, 3, 2, 3), 0, 0)
        IMAGE = put(IMAGE, CloseButton, 2, 2)
        IMAGE = put(IMAGE, HideButton, 2, height-3, "02")
        IMAGE = put(IMAGE, tile(Studs, 8, height-14-15), 3, 14)
    return IMAGE
...
def stretch(size, amount, c):
    # this is needed because deform() does the opposite of what you would think it will do, it takes 4 points, and then squishes them into a rectangle.
    result = size-size*(size/(size-size/amount))
...
def Create7Glass(width, height, active=True):
    ...
    Glass = put(Image.new("RGBA", (800, 602), (0, 0, 0, 0)), GlassImg.resize(
        screenres, 0), int(-pos[0]+width/16-screenres[0]/16+pos[0]/8), -pos[1])
    ...
    WithBorder = put(WithBorder, SideShine.resize(
        (w(SideShine), (height-29-8)//4)), 0, 29)
    WithBorder = put(WithBorder, SideShine.resize(
        (w(SideShine), (height-29-8)//4)), width, 29, "20")
    # WithBorder.show()
    # if(title != ""):
    ...
    IMAGE = put(IMAGE, resize(ShadowRight, 20,
                height, 1, 1, 17, 17), width+14, 14)
    IMAGE = put(IMAGE, resize(ShadowBottom, width +
                34, 20, 30, 29, 1, 1), 0, height+14)
...
def Create7Window(icon="", text="", title="", active=True, buttons=[]):
    ...
    for i in buttons:
        ...
        CONTENT = put7(CONTENT, Button, contentwidth -
...
def Create7TaskDialog(icon="", textbig="", textsmall="", title="", buttons=[], closebutton=True, active=True, pos=(200, 100), screenres=(1920, 1080), wallpaper=""):
    ...
    if buttons:
        ...
        CONTENT = put(CONTENT, Image.new("RGBA", (width, 1),
                      (222, 222, 222, 255)), 0, height-41)
    buttonpos = 12
    for button in buttons:
    ...
    Glass = put(Image.new("RGBA", (800, 602), (0, 0, 0, 0)), GlassImg.resize(
        screenres), int(-pos[0]+width/16-screenres[0]/16+pos[0]/8), -pos[1])
    WithBorder = ImageChops.multiply(GlassMask, Glass)
    WithBorder = put(WithBorder, SideGlowLeft, 0, 0)
    WithBorder = put(WithBorder, SideGlowRight, width, 0, "20")
    WithBorder = put(WithBorder, SideShine.resize(
    ...
    IMAGE = put(IMAGE, resize(ShadowRight, 19,
                height, 1, 1, 20, 14), width+13, 12)
    IMAGE = put(IMAGE, resize(ShadowBottom, width +
...
def Create3_1Window(icon="", text="", title="", buttons=[], active=True):
    ...
    buttonswidth = 0
    for button in buttons:
        CurrentButton = Create3_1Button(
            button[0], button[1], getsafe(button, 2, False))
    ...
    if active:
        for i in range(len(buttons)):
            CONTENT = put(CONTENT, Create3_1Button(buttons[i][0], buttons[i][1], getsafe(
                buttons[i], 2, False)), buttoneven(buttonpos), contentheight-10, "02")
            buttonpos += 58+17
    else:
        for i in range(len(buttons)):
            CONTENT = put(CONTENT, Create3_1Button(buttons[i][0], 0, getsafe(
                buttons[i], 2, False)), buttoneven(buttonpos), contentheight-10, "02")
            buttonpos += 58+17
...
def Create95Window(icon="", text="", title="", buttons=[], active=True, closebutton=True):
    ...
    if (buttons):
        ...
        ButtonImg = Create95Button(button[0], getsafe(
            button, 1, 0) if active else 0, getsafe(button, 2, False))
        ButtonsImg = put(Image.new("RGBA", (w(ButtonsImg)+w(ButtonImg),
                         max(h(ButtonsImg), h(ButtonImg))), (0, 0, 0, 0)), ButtonsImg, 0, 0)
        ButtonsImg = put(ButtonsImg, ButtonImg, w(ButtonsImg), 0, "20")
        buttons.pop(0)
        for button in buttons:
            ButtonImg = Create95Button(button[0], getsafe(
                button, 1, 0) if active else 0, getsafe(button, 2, False))
            ButtonsImg = put(Image.new("RGBA", (w(ButtonsImg)+w(ButtonImg)+6,
                             max(h(ButtonsImg), h(ButtonImg))), (0, 0, 0, 0)), ButtonsImg, 0, 0)
            ButtonsImg = put(ButtonsImg, ButtonImg, w(ButtonsImg), 0, "20")
    ...
    if (text):
        ...
        IMAGE = put(IMAGE, TextImg, 18+textshift, 21 if h(TextImg)
                    == 13 else 16 if h(TextImg) == 26 else 12)
    if (buttons):
...
def Create98Window(icon="", text="", title="", buttons=[], active=True, closebutton=True, gradient1active=(0, 0, 128), gradient2active=(16, 132, 208), gradient1inactive=(128, 128, 128), gradient2inactive=(181, 181, 181)):
    ...
    IMAGE = put(resize(Window, width+2+2, height +
                21+2, 3, 3, 21, 2), IMAGE, 2, 21)
    if active:
        IMAGE = put(IMAGE, Image.new(
            "RGBA", (width-2, 18), gradient2active), 3, 3)
        IMAGE = put(IMAGE, gradient(width-2-19, 18,
                    gradient1active, gradient2active), 3, 3)
    else:
        IMAGE = put(IMAGE, Image.new(
            "RGBA", (width-2, 18), gradient2inactive), 3, 3)
        IMAGE = put(IMAGE, gradient(width-2-19, 18,
...
def Create2000Window(icon="", text="", title="", buttons=[], active=True, closebutton=True):
    ...
    if (buttons):
        button = buttons[0]
        ButtonsImg = Image.new("RGBA", (1, 1), (0, 0, 0, 0))
        ButtonImg = Create2000Button(button[0], getsafe(
            button, 1, 0) if active else 0, getsafe(button, 2, False))
        ButtonsImg = put(Image.new("RGBA", (w(ButtonsImg)+w(ButtonImg),
                         max(h(ButtonsImg), h(ButtonImg))), (0, 0, 0, 0)), ButtonsImg, 0, 0)
        ButtonsImg = put(ButtonsImg, ButtonImg, w(ButtonsImg), 0, "20")
    ...
    if active:
        IMAGE = put(IMAGE, Image.new(
            "RGBA", (width-2, 18), (166, 202, 240)), 3, 3)
        IMAGE = put(IMAGE, gradient(width-2-19, 18,
                    (10, 36, 106), (166, 202, 240)), 3, 3)
    else:
        IMAGE = put(IMAGE, Image.new(
            "RGBA", (width-2, 18), (192, 192, 192)), 3, 3)
        IMAGE = put(IMAGE, gradient(width-2-19, 18,
...
def FrameXPWindow(image, title, active=True, close=1, maximize=1, minimize=1, question=0):
    ...
    IMAGE = put(IMAGE, resize(TopFrame, width, h(
        TopFrame), 28, 35, 9, 17, Image.NEAREST), 0, 0)
    IMAGE = put(IMAGE, LeftFrame.resize(
        (w(LeftFrame), height-h(TopFrame)-3), Image.NEAREST), 0, h(TopFrame), "00")
    IMAGE = put(IMAGE, RightFrame.resize((w(RightFrame), height -
                h(TopFrame)-3), Image.NEAREST), width, h(TopFrame), "20")
    IMAGE = put(IMAGE, cropx(BottomFrame, 0, 5), 0, height, "02")
    IMAGE = put(IMAGE, cropx(BottomFrame, 4, w(BottomFrame) -
                5).resize((width-10, h(BottomFrame)), Image.NEAREST), 5, height, "02")
    IMAGE = put(IMAGE, cropx(BottomFrame, w(BottomFrame) -
                5, w(BottomFrame)), width, height, "22")
...
#    o = Create7Glass(400,400)[0]
#    o.save("coolglass.png")
