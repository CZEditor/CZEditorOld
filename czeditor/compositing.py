...
# combined translation, rotation, and projection
def translaterotateproject(width, height, position, rotation, origin, corner, perspective=250):
    vec = np.array((width*corner[0]-(width*origin[0])/2,
                   height*corner[1]-(height*origin[1])/2, 0))
    rotated = rotate(vec, rotation)
    z = 1+rotated[2]/perspective+position[2]
    projected = [(rotated[0]-width*(0.5-origin[0]/2))/z+width/2+position[0],
                 (rotated[1]-height*(0.5-origin[1]/2))/z+height/2+position[1]]
    return projected
...
def CreateCustomWindowAnimation(image, time=1, startpos=(0, 0, 0), startrotation=(0, 0, 0), origin=(0.5, 0.5, 0)):
    ...
    NW = translaterotateproject(w(image), h(
        image), startpos*(1-t), startrotation*(1-t), origin, (0, 0, 0))
    NE = translaterotateproject(w(image), h(
        image), startpos*(1-t), startrotation*(1-t), origin, (1, 0, 0))
    SW = translaterotateproject(w(image), h(
        image), startpos*(1-t), startrotation*(1-t), origin, (0, 1, 0))
    SE = translaterotateproject(w(image), h(
        image), startpos*(1-t), startrotation*(1-t), origin, (1, 1, 0))
    coeffs = find_coeffs([NW, NE, SE, SW], [[0, 0], [w(image), 0], [
                         w(image), h(image)], [0, h(image)]])
    return coeffs


# warp image by coefficients and composite
def ExecuteCustomWindowAnimation(image, coeffs, time, wallpaper=None, pos=None, align=None):
    t = min(1, max(0, time))
    image = image.transform(
        image.size, Image.PERSPECTIVE, coeffs, Image.LINEAR)
    no = image.copy()
    no.putalpha(0)
...
def Composite7(img, GlassMask, time, startpos, startrotation, origin, wallpaper, pos, align):
    ...
    GlassMask = put(Image.new("RGBA", img.size,
                    (255, 255, 255, 0)), GlassMask, 14, 14)
    WithBorder = ImageChops.multiply(WithBorder, GlassMask)
    IMAGE = put(WithBorder, img, 0, 0)
    coeffs = CreateCustomWindowAnimation(
        IMAGE, time, startpos, startrotation, origin)
    ...
    masked = ImageChops.multiply(put(Image.new(
        "RGBA", wallpaper.size, (0, 0, 0, 0)), GlassMask, pos[0]-14, pos[1]-14, align), Blur)
