...
class CustomShader:
    ...
    def composite(image,vertices,shader,params,windowObject,keyframe,frame):
        ...
        if(transient.shader is None or params.custom() != transient.previousCustom or transient.previousIndex != index):
            ...
            transient.shader = compileShader("""#version 450 core
                void shadercustom"""+str(index)+"""(in vec2 inpos, float frame, vec3 worldpos, in float spectrum[1024], float variableA, float variableB, out vec2 outpos){
                    """+params.custom()+"""
                }
            """,GL_FRAGMENT_SHADER)
        ...
        shader.append({"fragmentshader":transient.shader,
                       ...
                       "fragmentdeclaration":"void shadercustom"+str(index)+"(in vec2 inpos, float frame, vec3 worldpos, in float spectrum[1024], float variableA, float variableB, out vec2 outpos);"})
        return image,vertices,shader
...
class CustomColorShader:
    ...
    def composite(image,vertices,shader,params,windowObject,keyframe,frame):
        ...
        if(transient.shader is None or params.custom() != transient.previousCustom or transient.previousIndex != index):
            ...
            transient.shader = compileShader("""#version 450 core
                vec4 shadercustom"""+str(index)+"""(in vec2 inpos, float frame, int width, int height, vec3 worldpos, in float spectrum[1024], in sampler2D image,float variableA, float variableB){
        ...
        shader.append({"fragmentshader":transient.shader,
                       ...
                       "fragmentdeclaration":"vec4 shadercustom"+str(index)+"(in vec2 inpos, float frame, int width, int height, vec3 worldpos, in float spectrum[1024], in sampler2D image,float variableA, float variableB);",
                       "ismultisample":True})
        return image,vertices,shader
...
class CustomVertexShader:
    ...
    def composite(image,vertices,shader,params,windowObject,keyframe,frame):
        ...
        if(transient.shader is None or params.custom() != transient.previousCustom or transient.previousIndex != index):
            ...
            transient.shader = compileShader("""#version 450 core
                void shadercustom"""+str(index)+"""(in vec3 inpos, in vec2 vertexColor, in float spectrum[1024], float variableA, float variableB, float frame, out vec3 outpos){
                    """+params.custom()+"""
                }
            """,GL_VERTEX_SHADER)
        ...
        shader.append({"vertexshader":transient.shader,
                       ...
                       "vertexdeclaration":"void shadercustom"+str(index)+"(in vec3 inpos, in vec2 vertexColor, in float spectrum[1024], float variableA, float variableB, float frame, out vec3 outpos);"})
        return image,vertices,shader
